...
public class People {
	...
	public void givenAddDeletePhone(int personId) throws Throwable{
    ...
    @When("^Hapus nomor telepon milik person dengan id person (\\d+) dan id phone (\\d+)$")
	public void whenDeletePhone(int personId, int phoneId) throws Throwable{
		StringBuilder jsonStr = new StringBuilder();
		jsonStr.append("{\"id\": ").append(phoneId).append("}");

		@SuppressWarnings("unused")
		HttpResponse<JsonNode> jsonResponse = Unirest.delete("http://localhost:8080/persons/delphone/"+phoneId)
				  .header("Content-Type", "application/json")
				  .header("Accept", "application/json")
				  .body(jsonStr.toString())
				  .asJson();
	...
	@Then("^Jumlah nomor telepon milik person dengan id (\\d+) menjadi jumlah awal - (\\d+)$")
	public void thenDeletePhone(int personId, int jmlPenguranganNoTelp) throws Throwable{
		HttpResponse<JsonNode> jsonResponse = Unirest.get("http://localhost:8080/persons/"+personId)
				.header("Accept", "application/json")
				.asJson();
		JSONObject objPerson  = jsonResponse.getBody().getObject();
		JSONArray arrPhoneNumber = objPerson.getJSONArray("phones");
		assertThat(arrPhoneNumber.length(),equalTo(jmlNoTelpAwal-jmlPenguranganNoTelp));
	}
}
